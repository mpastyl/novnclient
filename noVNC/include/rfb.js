...
that.sendCtrl = function() {
    if (rfb_state !== "normal" || conf.view_only) { return false; }
    Util.Info("Sending Ctrl");
    var arr = [];
    if (document.getElementById('noVNC_ctrl_box').checked) {
...
that.sendAlt = function() {
    if (rfb_state !== "normal" || conf.view_only) { return false; }
    Util.Info("Sending Alt");
    var arr = [];
    if (document.getElementById('noVNC_alt_box').checked) {
...
that.sendEnter = function() {
    if (rfb_state !== "normal" || conf.view_only) { return false; }
    Util.Info("Sending Enter");
    var arr = [];

    arr = arr.concat(keyEvent(0xFF0D, 1)); // Alt
    arr = arr.concat(keyEvent(0xFF0D, 0)); // Alt
    //arr = arr.concat(keyEvent(0xFFE3, 0)); // Control
    arr = arr.concat(fbUpdateRequests());
    ws.send(arr);
};
...
that.sendShift = function() {
    if (rfb_state !== "normal" || conf.view_only) { return false; }
    Util.Info("Sending Shift");
    var arr = [];
    if (document.getElementById('noVNC_shift_box').checked) {
    Util.Info("Sending Shift down (checked)");
    arr = arr.concat(keyEvent(0xFFE1, 1)); // Shift
    }
    else{
    Util.Info("Sending Shift up (unchecked)");
...
that.clipboardPasteFrom = function(text) {
    ...
    n=text.length;
    for (i=0; i < n; i+=1) {
        keyPress(text.charCodeAt(i),1);
        keyPress(text.charCodeAt(i),0);
        //alert(" i = "+i+"  val = "+text.charCodeAt(i));
        //alert('0x00'+text[i].charCodeAt(0).toString(16).toUpperCase());
        //alert(text.charCodeAt(i));
        //arr.concat(keyEvent(text.charCodeAt(i),1));
        //arr.concat(keyEvent(text.charCodeAt(i),0));
        //arr.concat(keyEvent('0x00'+text.charCodeAt(i).toString(16).toUpperCase(),1));
        //arr.concat(keyEvent('0x00'+text.charCodeAt(i).toString(16).toUpperCase(),0));
        //arr = arr.concat(fbUpdateRequests());
        //ws.send(arr);
        //arr = [];

    }
    //arr = arr.concat(keyEvent(0x0061, 1)); // Alt
    //arr = arr.concat(keyEvent(0x0061, 0)); // Alt
    //arr = arr.concat(keyEvent(0xFFE3, 0)); // Control
    //arr = arr.concat(fbUpdateRequests());
    //ws.send(arr);
